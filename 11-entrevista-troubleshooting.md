...
- **Aumentar número de consumers**: ganha throughput só se o gargalo for capacidade de processamento; perde eficácia (e pode piorar) se o gargalo for uma dependência externa já saturada.
- **Back pressure (limitar producer)**: ganha proteção contra estouro de memória/fila infinita; perde vazão — mensagens novas esperam ou são rejeitadas.
- **Filas separadas por prioridade**: ganha SLA garantido pro que é crítico; perde simplicidade operacional (mais filas para monitorar e manter).
---
...
- **DLQ com limite de tentativas**: ganha que a fila principal nunca trava numa mensagem só; perde uma mensagem 'presa' que exige investigação e reprocessamento manual.
- **Retry infinito sem DLQ (o que existia antes)**: nenhum ganho real; perde a fila inteira, porque uma mensagem problemática bloqueia todas as que vêm atrás — por isso não é uma alternativa viável, só o estado a evitar.
---
...
- **Idempotência via registro de IDs processados**: ganha correção robusta sem custo de mensageria mais cara; perde overhead de armazenar e checar IDs (mais uma escrita/leitura por mensagem).
- **Perseguir 'exactly-once' de ponta a ponta**: ganha (em teoria) nunca ter duplicata; perde viabilidade prática — é caro, complexo e a maioria dos sistemas reais não garante isso de forma genérica.
---
...
- **Aumentar o tamanho do pool (mitigação)**: ganha alívio imediato do sintoma; perde por não resolver a causa — o leak continua, só demora mais para estourar de novo.
- **Corrigir o leak na origem (fix estrutural)**: ganha resolução definitiva; perde tempo — exige revisão de código, testes e um novo deploy antes de eliminar o risco.
---
...
- **Ordem consistente de locks entre todo o código**: ganha eliminação estrutural da possibilidade de deadlock; perde flexibilidade — exige disciplina em toda parte do código que trava os mesmos recursos.
- **Retry automático em erro de deadlock**: ganha resiliência a impasses ocasionais; perde por não atacar a causa raiz — é um complemento, não substitui a correção da ordem de locks.
- **Saga (dividir em transações locais menores)**: ganha eliminar a necessidade de lock compartilhado entre serviços; perde simplicidade — é bem mais trabalhoso de implementar e exige lógica de compensação.
---
...
- **Sempre ler do master**: ganha consistência garantida; perde escalabilidade de leitura — volta a sobrecarregar o banco principal.
- **'Read your own writes' seletivo**: ganha equilíbrio (só quem acabou de escrever lê do master); perde alguma complexidade de roteamento de leitura.
- **Aceitar eventual consistency**: ganha simplicidade e escalabilidade máxima; perde experiência do usuário em casos sensíveis a atraso (chat, painel colaborativo).
---
...
- **Adicionar índice**: ganha leitura muito mais rápida; perde um pouco de velocidade de escrita (todo índice precisa ser mantido a cada INSERT/UPDATE).
- **Reescrever N+1 como JOIN/WHERE IN**: ganha eliminar dezenas/centenas de round-trips ao banco; perde simplicidade do código se a query resultante virar mais complexa de manter.
---
...
- **GC otimizado para baixa latência**: ganha pausas mais curtas e previsíveis; perde throughput agregado em alguns casos.
- **Aumentar o heap**: ganha adiar a necessidade de nova coleta; perde — quando a coleta finalmente acontecer, tende a ser mais demorada.
- **Migrar componente crítico para runtime sem GC (ex: Rust)**: ganha eliminação total de pausas stop-the-world; perde — reescrita cara e mais complexidade de gerenciamento manual de memória.
---
...
- **Deadline propagation (orçamento de tempo entre serviços)**: ganha consistência real do timeout ponta a ponta; perde esforço de implementação — precisa instrumentar toda a cadeia de chamadas.
- **Timeout fixo por serviço, sem propagação**: ganha simplicidade de implementar; perde adaptabilidade — pode estourar sem necessidade ou permitir esperas maiores que o cliente tolera.
---
...
- **Escalar horizontalmente**: ganha mitigação rápida do sintoma; perde eficácia se o problema for ineficiência de código, não falta de servidores — nesse caso só adia o teto.
- **Profiling + otimizar o hot path**: ganha resolver a causa raiz com melhor custo-benefício de longo prazo; perde velocidade — exige investigação antes de qualquer ganho.
- **Cache / rate limit / circuit breaker**: ganham reduzir carga desnecessária de forma estrutural; perdem tempo de implementação e exigem calibração fina para não rejeitar tráfego legítimo.
---
...
- **Modelo assíncrono/não-bloqueante**: ganha desacoplar concorrência de threads do SO, resolvendo o problema na raiz; perde — reescrita do código é mais complexa.
- **Aumentar o pool de threads**: ganha alívio rápido e simples; perde — tem teto (memória, overhead de troca de contexto) e o problema volta em volume ainda maior.
- **Timeout mais agressivo na dependência externa**: ganha liberar recursos mais rápido; perde — mais requisições retornam erro para o usuário final.
---
...
- **Rate limiting próprio (token bucket) antes de chamar o provedor**: ganha nunca estourar o limite contratado; perde — picos de demanda passam a esperar/enfileirar em vez de serem atendidos na hora.
- **Fila com vazão controlada**: ganha desacoplar demanda do usuário da capacidade do provedor; perde latência percebida (processamento deixa de ser síncrono do ponto de vista do usuário).
- **Negociar limite maior com o provedor**: ganha resolver na raiz sem mudança técnica; perde — depende de terceiro e normalmente tem custo financeiro maior.
---
...
- **Lock otimista (versão + retry)**: ganha baixo custo quando a concorrência real é rara; perde — sob altíssima concorrência gera muitas tentativas repetidas.
- **Lock pessimista (SELECT ... FOR UPDATE)**: ganha garantia direta de exclusão mútua; perde throughput — transações ficam esperando na fila do lock sob alta concorrência.
- **UPDATE atômico condicional no SQL**: ganha simplicidade máxima, sem lock explícito nem retry na aplicação; perde flexibilidade — não cobre bem lógica de negócio mais complexa envolvendo múltiplas tabelas na mesma transação.
---
...
- **Idempotency key com constraint de unicidade**: ganha correção robusta mesmo sob concorrência real (duas requisições simultâneas); perde — overhead de armazenar/checar chaves e depende do cliente gerar e reenviar a mesma chave corretamente.
---
...
- **Ordem consistente de aquisição de locks**: ganha eliminação matemática do ciclo de espera circular; perde — exige disciplina e documentação em todo o código que usa esses locks.
- **Lock com timeout**: ganha não travar para sempre; perde — não resolve a causa raiz, é mitigação (a operação ainda falha, só não trava o processo inteiro).
- **Reduzir estado mutável compartilhado (message passing/imutabilidade)**: ganha eliminar boa parte da necessidade de locks; perde — é uma mudança estrutural grande, com esforço de reescrita alto.
---
...
- **Restart programado (mitigação)**: ganha estabilidade previsível enquanto a causa não é corrigida; perde — não resolve nada, apenas esconde o sintoma.
- **Corrigir o leak na origem**: ganha resolução definitiva; perde tempo — exige heap dump, profiling e investigação mais demorada antes do fix.
---
...
- **Streaming (processar em pedaços)**: ganha memória praticamente constante, independente do tamanho do arquivo; perde — mais complexo de implementar quando a lógica de negócio precisa de uma visão do arquivo inteiro.
- **Limite máximo de tamanho de upload (mitigação)**: ganha proteção imediata contra o crash; perde — impede, mesmo que temporariamente, casos de uso legítimos com arquivos grandes.
---
...
- **Tuning do GC para baixa latência**: ganha pausas menores e mais previsíveis; perde throughput agregado em certos algoritmos.
- **Reduzir criação de objetos temporários no código**: ganha menos lixo gerado, coletas menos frequentes; perde — exige refatoração do código quente, que pode ser trabalhosa.
- **Migrar para runtime sem GC (casos extremos)**: ganha previsibilidade máxima de latência; perde — custo de reescrita e complexidade de gerenciamento manual de memória.
---
...
- **Lock/single-flight na repopulação**: ganha eliminar a rajada de queries no banco; perde — as demais requisições esperam brevemente pelo resultado do 'líder', em vez de serem atendidas na hora.
- **Refresh-ahead**: ganha a chave nunca 'ficar vazia'; perde trabalho computacional extra recorrente (recalcula antes de precisar de fato).
- **TTL com jitter**: ganha simplicidade de implementação; perde eficácia se o tráfego for extremamente concentrado numa única chave popular — sozinho pode não ser suficiente.
---
...
- **Cache compartilhado (Redis)**: ganha uma única fonte de verdade, simples de operar; perde a latência de rede do cache local e cria um ponto único de dependência.
- **Invalidação ativa (pub/sub)**: ganha manter a performance do cache local com consistência; perde complexidade de implementação (infraestrutura de mensagens entre instâncias).
- **Aceitar TTL curto (sem mudar arquitetura)**: ganha simplicidade máxima; perde — ainda existe uma janela de inconsistência, só que menor.
---
...
- **Nunca cachear resposta de erro**: ganha eliminar o problema quase por completo; perde — mais uma chamada ao banco a cada erro transitório em vez de servir do cache (efeito colateral aceitável).
- **Remover a chave manualmente (mitigação imediata)**: ganha resolver rápido para os usuários já afetados; perde — não é automático, exige intervenção humana a cada ocorrência até o fix definitivo.
---
...
- **Rollback imediato**: ganha mitigação rápida do impacto para o usuário; perde — a causa raiz fica sem investigação enquanto isso (mas geralmente é a priorização correta em incidente ativo).
- **Canary/rolling deploy**: ganha limitar o 'raio de impacto' de deploys ruins; perde velocidade/simplicidade de operação — deploys ficam mais lentos e exigem mais orquestração.
- **Feature flag**: ganha poder desligar instantaneamente sem novo deploy; perde — overhead de manter flags e código para múltiplos estados no sistema.
---
...
- **Testes de regressão para regras de negócio críticas**: ganham pegar o bug antes de chegar em produção; perdem tempo de manutenção contínua da suíte de testes.
- **Monitoramento de métricas de negócio (não só técnicas)**: ganha detectar anomalias rapidamente; perde — exige calibração cuidadosa para não gerar falsos positivos.
---
...
- **Migrations em etapas (expand/contract)**: ganha que um rollback de código nunca quebre por incompatibilidade de schema; perde velocidade — exige múltiplos deploys e mais disciplina de processo.
- **Migration destrutiva direta (o que causou o problema)**: ganha rapidez de implementação; perde segurança — qualquer rollback de código posterior corre risco real de quebrar.
---
...
- **Timeout + circuit breaker + bulkhead juntos**: ganham isolar a falha e impedir o efeito cascata; perdem simplicidade — mais configuração, mais métricas para acompanhar, mais superfície para calibrar mal.
- **Sem nenhum mecanismo de isolamento (o estado inicial do cenário)**: ganha simplicidade de código; perde — qualquer degradação de uma dependência pode derrubar toda a cadeia acima dela.
---
...
- **Backoff exponencial + jitter + limite de tentativas**: ganha evitar a amplificação de carga que gera o retry storm; perde — a recuperação de uma falha pontual fica um pouco mais lenta (aceita esperar mais entre tentativas).
- **Retry imediato sem backoff (o que causou o problema)**: ganha simplicidade; perde — sob degradação, multiplica o tráfego efetivo e piora a situação em vez de ajudar.
---
...
- **Versionamento/expand-contract de contrato de API**: ganha nunca quebrar consumidores existentes; perde — precisa manter campos/versões antigas temporariamente, com mais complexidade de manutenção.
- **Mudar o contrato direto, sem aviso (o que causou o problema)**: ganha rapidez para quem está mudando; perde — quebra qualquer consumidor que não seja avisado com antecedência.
---
...
- **Manter o índice**: ganha leitura muito mais rápida; perde velocidade de escrita — vale a pena quando a proporção leitura/escrita é alta o suficiente para compensar.
- **Remover o índice / mover consulta para réplica dedicada**: ganha recuperar a velocidade de escrita na tabela principal; perde — a consulta que dependia do índice fica mais lenta, ou exige infraestrutura extra (réplica) para continuar rápida.
---
...
- **Criar um índice adicional dedicado**: ganha atender bem os dois padrões de consulta; perde — mais um índice para manter, com custo de escrita adicional.
- **Não criar índice extra**: ganha nenhum custo de escrita a mais; perde — a consulta que filtra só pela segunda coluna continua lenta.
---
...
- **Remover índices não usados/redundantes**: ganha recuperar a performance de escrita do job em lote; perde — risco de remover algo usado raramente mas importante, se a validação em staging não for cuidadosa.
- **Manter todos os índices 'por precaução'**: ganha nenhuma query fica descoberta; perde — escrita seriamente degradada, como o próprio cenário demonstra.
---
...
- **Automatizar renovação (ex: Let's Encrypt)**: ganha eliminar completamente o risco de esquecimento humano; perde — exige configuração inicial de infraestrutura de automação.
- **Só monitoramento com alerta antecipado (sem automação)**: ganha tempo de reação antes do vencimento; perde — ainda depende de uma ação manual acontecer a tempo quando o alerta dispara.
---
...
- **Reduzir o TTL do DNS com antecedência**: ganha propagação muito mais rápida no momento da troca real; perde — mais consultas DNS (menos cache) no período anterior à mudança.
- **Manter o servidor antigo ativo durante a transição**: ganha zero downtime durante a janela de propagação; perde — custo de manter infraestrutura duplicada temporariamente.
---
...
- **Connection pooling / keep-alive**: ganha eliminar o custo repetido de resolução DNS e handshake em cada chamada; perde — conexões ociosas consomem recursos (memória, file descriptors) enquanto ficam mantidas abertas.
---
...
- **Rotação com overlap (chave antiga e nova válidas simultaneamente)**: ganha transição sem indisponibilidade; perde — janela de tempo em que duas chaves são válidas, uma superfície de segurança levemente maior.
- **Corte seco (invalidar a chave antiga imediatamente)**: ganha postura de segurança mais estrita; perde — alto risco de indisponibilidade se algum consumidor não tiver atualizado a tempo.
---
...
- **Sincronizar feature flags entre staging e produção por padrão**: ganha detectar bugs de comportamento antes de produção; perde um pouco de liberdade para testar funcionalidades isoladamente em staging.
- **Permitir divergência controlada entre ambientes**: ganha flexibilidade para testes incrementais; perde — risco real de 'funciona no staging, não em produção' se os dois estados da flag não forem testados.
---
...
- **Hash criptográfico bem distribuído para segmentação**: ganha distribuição verdadeiramente uniforme, mesmo com IDs sequenciais/correlacionados; perde — custo computacional marginalmente maior (irrelevante na prática).
- **Hash ingênuo (módulo simples sobre o ID)**: ganha simplicidade de implementação; perde — risco real de correlação indesejada com outras variáveis (país, data de cadastro), como o próprio cenário mostra.
---
